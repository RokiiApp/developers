...
describe('search', () => {
  ...
  it('searchs in objects array with toString function', () => {
    const items = [{ name: 'Facetime' }, { name: 'Terminal' }];
    const term = 'face';

    const toString = (x: typeof items[number]) => x.name;
...
describe('case insesetive', () => {
  ...
  const toString = (app: typeof items[number]) => `${app.name} ${app.filename}`;
